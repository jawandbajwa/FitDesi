...
function getHeightInCm() {
    ...
    if (isCm) {
        ...
        const inches = parseFloat(document.getElementById('userHeightIn').value) || 0;
...
function getEditWeightInKg() {
    const isKg = document.getElementById('editWeightKg').classList.contains('active');
    if (isKg) {
        return parseFloat(document.getElementById('editWeight').value) || 0;
    } else {
        const lbs = parseFloat(document.getElementById('editWeightLbsVal').value) || 0;
        return Math.round(lbs * 0.453592 * 10) / 10;
    }
}

function getEditHeightInCm() {
    const isCm = document.getElementById('editHeightCm').classList.contains('active');
    if (isCm) {
        return parseFloat(document.getElementById('editHeight').value) || 0;
    } else {
        const ft = parseFloat(document.getElementById('editHeightFtVal').value) || 0;
        const inches = parseFloat(document.getElementById('editHeightInVal').value) || 0;
        return Math.round((ft * 30.48) + (inches * 2.54));
    }
}
...
function generateCoachMessage(profile, macros, consumed, recipes) {
    ...
    const goalLabel  = goal === 'recomp' ? 'body recomposition' : goal === 'muscle' ? 'muscle gain' : 'fat loss';
    ...
    if (hour < 11) {
        ...
        if (consumed.protein === 0) message += `You haven't logged breakfast yet — start strong. `;
        if (highProteinRecipes) message += `Try ${highProteinRecipes} from your cookbook for a protein-rich start.`;
    } else if (hour < 15) {
        ...
        if (remaining.protein > 60) {
            ...
            if (highProteinRecipes) message += `${highProteinRecipes} would be great choices.`;
        ...
        if (remaining.calories < 300) {
            ...
            if (highProteinRecipes) message += `For dinner, ${highProteinRecipes} would help close the gap.`;
        }
    } else {
...
function renderMacroRings(macros, consumed) {
    const items = [
        { label: 'Protein',  consumed: consumed.protein,  target: macros.protein,  unit: 'g'    },
        { label: 'Carbs',    consumed: consumed.carbs,    target: macros.carbs,    unit: 'g'    },
        { label: 'Fat',      consumed: consumed.fat,      target: macros.fat,      unit: 'g'    },
        { label: 'Calories', consumed: consumed.calories, target: macros.calories, unit: 'kcal' }
    ];
    document.getElementById('macroRings').innerHTML = items.map(item => {
...
document.querySelectorAll('.planner-cuisine-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        document.querySelectorAll('.planner-cuisine-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
...
function renderPlannerTargets(macros, consumed) {
    ...
    const targets = {
        protein:  isRemaining ? Math.max(0, macros.protein  - Math.round(consumed.protein))  : macros.protein,
        carbs:    isRemaining ? Math.max(0, macros.carbs    - Math.round(consumed.carbs))    : macros.carbs,
        fat:      isRemaining ? Math.max(0, macros.fat      - Math.round(consumed.fat))      : macros.fat,
        calories: isRemaining ? Math.max(0, macros.calories - Math.round(consumed.calories)) : macros.calories,
    };
    ...
    `;
    document.getElementById('plannerTargets').dataset.targets = JSON.stringify(targets);
}
...
document.getElementById('generatePlanBtn').addEventListener('click', async () => {
    ...
    const targetsRaw = document.getElementById('plannerTargets').dataset.targets;
    if (!targetsRaw) { btn.disabled = false; btn.textContent = '✨ Generate Plan'; return; }
    ...
    if (!breakfasts.length || !lunches.length || !dinners.length || !snacks.length) {
        alert('Not enough recipes in the database. Add more recipes in the admin panel first.');
        btn.disabled = false;
    ...
    function scoreCombination(b, l, d, s) {
        const total = {
            protein:  (b.protein  || 0) + (l.protein  || 0) + (d.protein  || 0) + (s.protein  || 0),
            carbs:    (b.carbs    || 0) + (l.carbs    || 0) + (d.carbs    || 0) + (s.carbs    || 0),
            fat:      (b.fat      || 0) + (l.fat      || 0) + (d.fat      || 0) + (s.fat      || 0),
            calories: (b.calories || 0) + (l.calories || 0) + (d.calories || 0) + (s.calories || 0),
        };
        return (
    ...
    for (const b of bS) for (const l of lS) for (const d of dS) for (const s of sS) {
        const score = scoreCombination(b, l, d, s);
        if (score < bestScore) { bestScore = score; bestCombo = { b, l, d, s }; }
    }
...
function renderPlannerResults(targets) {
    ...
    document.getElementById('plannerMeals').innerHTML = meals.map(meal => {
...
function updatePlannerTotals(targets) {
    ...
    function diffLabel(actual, target) {
        ...
        if (diff > 0) return `<div class="planner-total-diff over">+${diff} over</div>`;
    ...
    `;
    document.getElementById('plannerTotals').dataset.targets = JSON.stringify(targets);
}

function openSwapModal(meal) {
    swappingMeal = meal;
    document.getElementById('swapModalLabel').textContent = `Choose a different ${meal}`;
    const options = plannerRecipes.filter(r =>
        r.category === meal && r.id !== currentPlan[meal]?.id
    );
    document.getElementById('swapList').innerHTML = options.length > 0
        ? options.map(r => `
    ...
    document.querySelectorAll('.swap-item').forEach(el => {
        el.addEventListener('click', () => {
            ...
            if (selected) {
                ...
                const targets = JSON.parse(document.getElementById('plannerTotals').dataset.targets || '{}');
                renderPlannerResults(targets);
...
document.addEventListener('click', async function(e) {
    ...
    if (e.target.closest('.goal-btn')) {
        ...
        if (field === 'editGoal') {
            document.querySelectorAll('.goal-btn[data-field="editGoal"]').forEach(b => b.classList.remove('selected'));
            btn.classList.add('selected');
            editGoal = btn.dataset.val;
        } else {
            document.querySelectorAll('.goal-btn:not([data-field])').forEach(b => b.classList.remove('selected'));
            btn.classList.add('selected');
    ...
    if (e.target.id === 'toStep3') {
        ...
        if (!selectedActivity) { alert('Please select your activity level.'); return; }
        const profile = {
        ...
        `;
        document.getElementById('coachIntro').textContent = `Here are your personalized daily targets, ${profile.name}`;
        document.getElementById('coachMessage').textContent = generateOnboardingCoachMessage(profile, macros);
        document.getElementById('step2').classList.add('hidden');
    ...
    if (e.target.id === 'editProfileBtn') {
        ...
        document.querySelectorAll('.option-btn[data-field="editGender"]').forEach(b => {
            b.classList.toggle('selected', b.dataset.val === profile.gender);
        });
        document.querySelectorAll('.option-btn[data-field="editActivity"]').forEach(b => {
            b.classList.toggle('selected', b.dataset.val === profile.activity);
        });
        document.querySelectorAll('.goal-btn[data-field="editGoal"]').forEach(b => {
    ...
    if (e.target.id === 'saveEditProfile') {
        ...
        if (!name || !age || !weight || !height || !editGender || !editGoal || !editActivity) {
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (document.getElementById('notifBreakfast')) {
        document.getElementById('notifBreakfast').value = notifTimes.breakfast || '08:00';
