...
import { getUserProfile, addMealToLog, completeWorkout, swapExercise } from './firebase.js';
...
function sendWelcomeMessage() {
  const message = {
    role: 'assistant',
    content: "Hey Jawand! I'm your personal coach. How's your day going? Ready to crush some goals?"
  };
...
async function sendMessage(text = null) {
  ...
  try {
    ...
    addMessage({ role: 'assistant', content: 'Sorry, I\'m having trouble connecting. Try again?' });
  }
}
...
function getQuickRepliesForContext(message) {
  ...
  if (message.toLowerCase().includes('tired') || message.toLowerCase().includes('rest')) {
    return ['Need a quick protein hit', 'Skip workout today', 'Light cardio instead'];
  }
  if (message.toLowerCase().includes('meal') || message.toLowerCase().includes('eat')) {
...
{"action":"swap_exercise","old":"...","new":"..."}`;
  ...
  payload.messages.unshift(...messages.slice(0, -1).map(m => ({
    role: m.role,
    content: m.content
  })));
